...
for (const name of Object.keys(SPECIES)) {
  ...
  for (let i = 0; i < 300; i++) {
    ...
    minClumps = Math.min(minClumps, b.foliage.length)

    const [, trunkTopY] = tipOf(b.trunk[0])
    const floor = crownFloor(b.foliage)
    // positive gap = daylight between the trunk top and the underside of the crown
    worstGap = Math.max(worstGap, floor - trunkTopY)
    assert(crownCeil(b.foliage) > trunkTopY, `${name}: crown must sit above the trunk top`)
    ...
    for (const e of [...b.foliage, ...b.trunk, ...b.branch]) {
...
for (let i = 0; i < 60; i++) {
  ...
  for (let a = 0; a < N; a++) {
    for (let c = 0; c < N; c++) {
      ...
      // plan-view coverage: does some clump's disc contain this soil sample
      if (b.foliage.some((e) => Math.hypot(e[0] - px, e[2] - pz) < e[3])) hit++
    }
  }
  const cover = hit / (N * N)
  worstCover = Math.min(worstCover, cover)
  assert(cover > 0.96, `bed ${w.toFixed(1)}x${d.toFixed(1)} leaves ${((1 - cover) * 100) | 0}% soil bare`)
  ...
  for (const e of [...b.flowerWhite, ...b.flowerPink, ...b.flowerYellow]) {
    ...
    assert(e[3] < 0.1, `bloom radius ${e[3].toFixed(3)} is bigger than the leaf it sits on`)
