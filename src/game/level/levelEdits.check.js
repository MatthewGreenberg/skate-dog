...
ok(TABLES.LAMPS.length === 10 && new Set(TABLES.LAMPS.map((l) => l.z)).size === 2, 'Dog Bowling lines both long sides with ten lamp posts')
ok(TABLES.LAMPS.every((l) => l.glow === '#26d9ff' || l.glow === '#ff4fd8'), 'Dog Bowling alternates cyan and pink lamp light')
ok(Object.entries(TABLES).every(([table, rows]) => table === 'CANS' || table === 'LAMPS' || rows.length === 0), 'Dog Bowling contains only its cans and sideline lamps')
ok(SCENE.time === 'neon' && SCENE.ground === 'asphalt', 'Dog Bowling uses the neon-night scene')
